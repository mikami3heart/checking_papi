...
//
	...
	//	"OFFCORE_RESPONSE_0:OTHER:L3_HITMESF:SNP_ANY",
	//	"OFFCORE_RESPONSE_0:OTHER:L3_MISS:SNP_ANY",
...
*/
	...
	if(retval != PAPI_OK) {
		fprintf(stderr,"*** <my_papi_add_events> failed for rank=%d\n", my_id);
		} else {
		fprintf(stderr,"<my_papi_add_events> returned PAPI_OK \n");
		}

	retval = my_papi_bind_start (papi.values, papi.num_events);
	if(retval != PAPI_OK) {
		fprintf(stderr,"*** <my_papi_bind_start> failed for rank=%d\n", my_id);
		} else {
		fprintf(stderr,"<my_papi_bind_start> returned PAPI_OK \n");
		}
//	t1=MPI_Wtime();
	fprintf (stderr,"<my_papi_bind_start> papi.num_events=%d\n", papi.num_events);
	for (int i=0; i<papi.num_events; i++) { fprintf (stderr,"#%d, %x %lld\n", i, papi.events[i], papi.values[i]); }
...
//	if(my_id == 0) { fprintf(stderr,"step %d finished in %f seconds\n", i,(t2-t1));}
	...
	fprintf(stderr,"Process %d finishing up\n", my_id);
//	MPI_Barrier(MPI_COMM_WORLD);
//	MPI_Finalize();
return 0;
}
